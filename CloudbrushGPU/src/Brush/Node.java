...
public class Node
{
	...
	// node members
	public String nodeid; //changed to public to workaround a Rootbeer error
	...
	public void setFields(Map< String, List< String >> f)
	{
		fields = f;
	}
	...
	{
		if ( et.equals( "ff" ) || et.equals( "fr" ) || et.equals( "rr" )
				|| et.equals( "rf" ) )
		{
	...
	public boolean hasEdge( String et, String nid, int oval_size )
			throws IOException
	{
		List< String > edges = getEdges( et );
		if ( edges == null )
		...
		{
			String[] vals = v.split( "!" );
			if ( vals[ 0 ].equals( nid )
					&& Integer.parseInt( vals[ 1 ] ) == oval_size )
			{
				return true;
			}
		}
	...
	public void removelink( String id, String dir ) throws IOException
	{
		...
		if ( edges != null )
		{
			for ( int i = 0; i < edges.size(); i++ )
			{
				...
				if ( idx == -1 )
				{
					throw new IOException( "Error removing link from " + getNodeId()
	...
	public void removelink( String id, String dir, int oval_size )
			throws IOException
	{
		boolean found = false;
		...
		if ( edges != null )
		{
			for ( int i = 0; i < edges.size(); i++ )
			{
				...
				/*
				 * int idx = edges.get(i).indexOf("!"); if (idx == -1) { throw new
				 * IOException("Error removing link from " + getNodeId() +
				 * ": Edge without overlap size information!! "); }
				 */
				if ( vals[ 0 ].equals( id )
						&& Integer.parseInt( vals[ 1 ] ) == oval_size )
	...
	public void removelink_update( String id, String dir, int oval_size )
			throws IOException
	{
	...
	public void replacelink_oval( String o, String ot, String n, String nt )
			throws IOException
	{
		// System.err.println(nodeid_m + " replacing " + o + ":" + ot + " => " + n +
		// ":" + nt);
		...
		if ( !found )
		{
			throw new IOException( nodeid + " Couldn't find link " + o + " " + ot
					+ " replace link " + n + " " + nt );
		}
	...
	public void replacelink( String o, String ot, String n, String nt )
			throws IOException
	{
		// System.err.println(nodeid_m + " replacing " + o + ":" + ot + " => " + n +
		// ":" + nt);
	...
	// \\// need modify
	public static String str_concat( String astr, String bstr, int K )
			throws IOException
	{
		// System.err.println("Str_len: " + astr.length() + " Oval_len:" + K +
		// " astr:" + astr + " bstr:" + bstr);
		// String as = astr.substring(astr.length()-K);
		// String bs = bstr.substring(0, K);

		/*
		 * if (!as.equals(bs)) { throw new IOException("as (" + as + ") != bs (" +
		 * bs + ")"); }
		 */
	...
	public void addBubble( String minor, String vmd, String vid, String umd,
			String uid, float extracov, int oval_size )
	{
		String msg = minor + "|" + vmd + "|" + vid + "|" + umd + "|" + uid + "|"
				+ extracov + "|" + oval_size;
	...
	public void addRemovalEdge( String node, String dir, String edge,
			int oval_size )
	{
		String msg = node + "|" + dir + "|" + edge + "|" + oval_size;
	...
	/*
	 * public Color getFColor(){ return Color.valueOf(fields.get(FCOLOR).get(0));
	 * }
	 * 
	 * public Color getRColor(){ return Color.valueOf(fields.get(RCOLOR).get(0));
	 * }
	 */
	...
	/*
	 * public void setFColor(Color color){ List<String> l = getOrAddField(FCOLOR);
	 * l.clear(); l.add(color.toString()); }
	 * 
	...
	class BlackEdgeComparator implements Comparator< String >
	...
	class BlackEdgeComparator2 implements Comparator< String >
	{ // possibly change to Comparator<String>?
		public int compare( String element1, String element2 )
		{
			String obj1 = element1;
			...
			if ( (int) ( ( Node.dna2str( val1[ 2 ] ).length() * Float
					.parseFloat( val1[ 4 ] ) ) - ( Node.dna2str( val2[ 2 ] ).length() * Float
					.parseFloat( val2[ 4 ] ) ) ) > 0 )
			{
				return -1;
			}
	...
	public static boolean fastcomparsion( String word1, String word2 )
	{
		...
		if ( word1.length() > 1000 )
		{
			...
			for ( int i = 0; i < 100; i++ )
			{
				if ( word1.substring( interval * i, interval * ( i + 1 ) ).equals(
						word2.substring( interval * i, interval * ( i + 1 ) ) ) )
				{
					count = count + 1;
				}
			}
			if ( word1.substring( interval * 99 ).equals(
					word2.substring( interval * 99 ) ) )
			{
				count = count + 1;
			}
	...
	public String selectBlackEdge( String dir )
	{
		...
		if ( black_edges != null )
		{
			Collections.sort( black_edges, new BlackEdgeComparator() ); // TODO: add
																																	// type safety
																																	// to
																																	// Comparison
			...
			if ( edge_list.size() >= 2 )
			{
				int consensus_len = edge_list.get( 1 )
						.substring( 0, edge_list.get( 1 ).indexOf( "!" ) ).length();
				...
				for ( int i = 0; i < edge_list.size(); i++ )
				{
					...
					{
						array[ j ][ 4 ] = array[ j ][ 4 ]
								+ (int) Float.parseFloat( compare_edge[ 1 ] );
				...
				if ( (float) N_count / (float) consensus.length() > 0.4
						&& consensus_len >= 20 )
				{
					return null;
				}
				// \\ Select best edge
				Collections.sort( black_edges, new BlackEdgeComparator2() ); // TODO:
																																			// Add
																																			// type
																																			// safety
				for ( int i = 0; i < black_edges.size(); i++ )
				{
					...
					if ( vals[ 0 ].equals( dir + "f" ) || vals[ 0 ].equals( dir + "r" ) )
					{
						...
						break;
						/*
						 * String str = Node.dna2str(vals[2]); if (vals[0].charAt(1) ==
						 * 'r'){ str = Node.rc(str); } String str_cut = ""; str_cut =
						 * str.substring(Integer.parseInt(vals[3])); int count_match = 0;
						 * int compare_len = 0; for(int j=0; j < str_cut.length() && j <
						 * consensus_len; j++ ) { if (str_cut.charAt(j) ==
						 * consensus.charAt(j)) { count_match = count_match + 1; }
						 * compare_len = j; } if ((float)count_match/(float)compare_len >
						 * 0.5) { return black_edges.get(i); }
						 */
					}
					// \\
	...
	public static String Consensus( List< String > edge_list, float majority,
			float threshold )
	{
		String consensus = null;
		class ConsensusComparator implements Comparator< String >
		{ // Added <String>
			public int compare( String element1, String element2 )
		...
		// \\\
		Collections.sort( edge_list, new ConsensusComparator() ); // TODO: Add type
																															// safety
		if ( edge_list.size() >= 2 )
		{
			...
			int consensus_len;
			if ( edge_list.size() == 2
					|| Float.parseFloat( compare_edge0[ 1 ] )
							+ Float.parseFloat( compare_edge1[ 1 ] ) > 2 )
			{
				consensus_len = edge_list.get( 1 )
						.substring( 0, edge_list.get( 1 ).indexOf( "!" ) ).length();
			}
			else
			{
				consensus_len = edge_list.get( 2 )
						.substring( 0, edge_list.get( 2 ).indexOf( "!" ) ).length();
			}
			// \\ 0:A 1:T 2:C 3:G 4:Sum
			int[][] array = new int[ consensus_len ][ 5 ];
			for ( int i = 0; i < consensus_len; i++ )
		...
		{
			return edge_list.get( 0 )
					.substring( 0, edge_list.get( 0 ).indexOf( "!" ) );
		}
		// \\\
		return consensus;
	}
	...
	public void addBlackEdge( String con, String node_id, String str_raw,
			int oval_size, float cov, String end )
	{
		String msg = con + "|" + node_id + "|" + str_raw + "|" + oval_size + "|"
				+ cov + "|" + end;
	...
	public boolean isGray( String dir ) throws IOException
	{
		...
		if ( black_edges != null )
		{
			...
			{
				String[] vals = black_edges.get( i ).split( "\\|" );
				if ( vals[ 0 ].equals( Node.flip_dir( dir ) + "r" )
						|| vals[ 0 ].equals( Node.flip_dir( dir ) + "f" ) )
				{
					black_id.add( vals[ 1 ] );
				}
			}
	...
	{
		/*
		 * List<String> l = getOrAddField(MATE); if (!l.contains(v)){ l.add(v); }
		 */
	}
	...
	{
		/*
		 * List<String> l = getOrAddField(MATE); if (l == null){ fields.put(MATE,
		 * filterAllPair(reads)); } else { l.addAll(reads); fields.put(MATE,
		 * filterAllPair(l)); }
		 */
	}
	...
	{
		List< String > verify_reads = new ArrayList< String >(); // added <String>
																															// for type
																															// safety.
	...
	public void setPairEnds( List< String > reads )
	{
		 ...
		 * 0){ fields.remove(MATE); } else { fields.put(MATE, filterAllPair(reads));
		 * } }
		 */
	}
	...
	{
		List< String > tmp = new ArrayList< String >(); // added String for type
																										// safety
	...
	public String toNodeMsg( boolean tagfirst )
	{
		...
		/*
		 * if (fields.containsKey(THREAD)) { sb.append("\t*"); sb.append(THREAD);
		 * for(String t : fields.get(THREAD)) { sb.append("\t"); sb.append(t); } }
		 */
		...
		/*
		 * if (fields.containsKey(R5)) { sb.append("\t*"); sb.append(R5); for(String
		 * r : fields.get(R5)) { sb.append("\t"); sb.append(r); } }
		 */

		/*
		 * if (fields.containsKey(BUNDLE)) { sb.append("\t*"); sb.append(BUNDLE);
		 * for(String r : fields.get(BUNDLE)) { sb.append("\t"); sb.append(r); } }
		 */
